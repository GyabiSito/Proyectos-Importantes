...
if (isset($_SESSION['usuario'])) {
    if (isset($_GET['id'])) {
        $stmt = $pdo->prepare('SELECT * FROM imagenes WHERE id_img=?');
        $stmt->execute([$_GET['id']]);
        $img = $stmt->FETCH(PDO::FETCH_ASSOC);
        if (!$img) {
            exit('la imágen seleccionada no existe');
        }
        if (isset($_GET['confirm'])) {
            if ($_GET['confirm'] == 'yes') {
                $stmt = $pdo->prepare("DELETE FROM imagenes WHERE id_img=?");
                $stmt->execute([$_GET['id']]);
                $msg = '<p class="subtitulo">imágen Eliminada</p>';
            }
        }
    } else {
        exit('No existe esta imágen');
    }
...
?>
 <!DOCTYPE html>
    ...
    <header>
        ...
        <main>
            <section>
                <h2 class="titulo">Eliminar "<?= $img['nombre'] ?>"</h2>
                <div class="contenedor">
                    <div class="ocultar">
                        <?php if ($msg) : ?>
                            <p><?= $msg ?></p>
                            <div class="volver">
                            <a href="mostrarimgModi.php?id=<?= $img['id_propiedades'] ?>">Volver a lista de registros</a>
                            </div>
                            
                        <?php else : ?>
                            <p class="subtitulo">¿Confirma que desea eliminar esta Imágen?</p>
                            <div class="yesno">
                                <a href="eliminarimg.php?id=<?= $img['id_img'] ?>&confirm=yes">Si</a>
                                <a href="mostrarimgModi.php?id=<?= $img['id_propiedades'] ?>&confirm=no">No</a>
                            </div>
                        <?php endif ?>
                    </div>
                </div>
