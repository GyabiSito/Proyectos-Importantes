...
if (isset($_SESSION['usuario'])) {
    if (isset($_GET['id'])) {
        ...
        if (isset($_GET['confirm'])) {
            if ($_GET['confirm'] == 'yes') {
                $stmt = $pdo->prepare("UPDATE propiedad set disponibilidad='no' WHERE id=?");
                $stmt->execute([$_GET['id']]);
                $id=$_GET['id'];
                $ci=$_SESSION['id_ci'];
                $datos=$pdo->prepare("INSERT INTO sesion (id_propiedad, id_ci, accion) VALUES ($id, $ci, 'Ocultó una propiedad')");
                $datos->execute(); 
                $msg = '<p class="subtitulo">Propiedad ocultada</p>';
            } else {
...
?>
    ...
    <head>
        ...
        <title>Ocultar Propiedad - Gestión Inmobiliaria</title>
    ...
    <body>
        ...
        <main>
            <section>
                <h2 class="titulo">Ocultar "<?= $propiedad['nombre'] ?>"</h2>
                <div class="contenedor">
                    <div class="ocultar">
                        ...
                        <?php else : ?>
                            <p class="subtitulo">¿Confirma que desea ocultar la propiedad?</p>
                            <div class="yesno">
                                <a href="ocultarpropiedad.php?id=<?= $propiedad['id'] ?>&confirm=yes">Si</a>
                                <a href="ocultarpropiedad.php?id=<?= $propiedad['id'] ?>&confirm=no">No</a>
                            </div>
                        <?php endif ?>
                    </div>
                </div>
