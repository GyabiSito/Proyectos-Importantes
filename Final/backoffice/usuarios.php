...
if (isset($_SESSION['usuario'])) {
    ...
    $records_per_page = 25;
    $stmt = $pdo->prepare('SELECT inmobiliario.ci, cuenta.id_ci, inmobiliario.nombre,inmobiliario.apellido, inmobiliario.contacto, cuenta.nickname, cuenta.permisos FROM inmobiliario JOIN cuenta  ON inmobiliario.ci = cuenta.id_ci LIMIT :current_page, :records_per_page');
    $stmt->BindValue(':current_page', ($page - 1) * $records_per_page, PDO::PARAM_INT);
    $stmt->BindValue('records_per_page', $records_per_page, PDO::PARAM_INT);
    $stmt->execute();
    $logs = $stmt->fetchall(PDO::FETCH_ASSOC);

    $num_contacts = $pdo->query('SELECT count(*) FROM inmobiliario')->fetchColumn();
?>
    <!DOCTYPE html>
    <html lang="en">
    ...
    <head>
        ...
        <title>Usuarios en el Sistema - Gestión Inmobiliaria</title>
    ...
    <body>
        ...
        <main>
            <section class="registros">
                <h2 class="titulo">USUARIOS EN EL SISTEMA</h2>
                            ...
                            <tbody>
                                <?php foreach ($logs as $log) : ?>
                                    <tr>
                                        ...
                                        <td><?= $log['permisos'] ?></td>
                                        <td class="actions">
                                            <a href="eliminarusu.php?id=<?= $log['id_ci'] ?>" class="trash">
